...
public:
    ...
    Gtkmm3OnPlayerUpdatedSignal(
        ...
        std::vector<std::unique_ptr<cx::cmn::ui::IColorPicker>>& p_playerChipColors)
    ...
    [[nodiscard]] std::unique_ptr<cx::cmn::ui::IConnection> Connect(const std::function<void()>& p_slot) override
    {
        ...
        for(std::unique_ptr<cx::cmn::ui::IColorPicker>& control : m_playerChipColors)
        {
            IF_CONDITION_NOT_MET_DO(control, continue;);
...
private:
    ...
    std::vector<std::unique_ptr<cx::cmn::ui::IColorPicker>>& m_playerChipColors;
