...
ConfigurableMainWindowPresenterTestFixture::ConfigurableMainWindowPresenterTestFixture()
{
    ...
    m_presenter = std::make_unique<cx::ui::MainWindowPresenter>(*m_modelAsLimits, *m_modelAsGameInformation, *m_modelAsUndoRedo, *m_modelAsAI);
    EXPECT_TRUE(m_presenter);
}

cx::ui::IMainWindowPresenter& ConfigurableMainWindowPresenterTestFixture::GetPresenter()
{
    EXPECT_TRUE(m_presenter);
    return *m_presenter;
}
